...
<main class="main-content" style="margin-left:280px;padding:40px;box-sizing:border-box;">
  <div class="container" style="max-width:800px;margin:0 auto;">
    <header class="page-header" style="margin-bottom:24px;">
      ...
      <h1 style="margin:0 0 6px 0;">Edit Tugas</h1>
      <p style="color:#6B7281;margin:0;">Perbarui informasi tugas</p>
    </header>
    ...
    <section class="form-card" style="background:#fff;border:1px solid #E5E7EB;border-radius:12px;padding:24px;box-shadow:0 1px 2px rgba(0,0,0,0.03);">
      <form action="{{ route('guru.kelas.tugas.update', [$teaching, $assignment]) }}" method="POST">
        @csrf
        @method('PUT')

        <div class="form-group" style="margin-bottom:16px;">
          <label for="title" style="display:block;font-weight:600;margin-bottom:8px;">Judul</label>
          <input type="text" id="title" name="title" class="form-input" value="{{ old('title', $assignment->title) }}" required style="width:100%;padding:10px 12px;border:1px solid #D1D5DB;border-radius:8px;">
        ...
        <div class="form-group" style="margin-bottom:16px;">
          ...
          <textarea id="description" name="description" rows="4" class="form-input" style="width:100%;padding:10px 12px;border:1px solid #D1D5DB;border-radius:8px;">{{ old('description', $assignment->description) }}</textarea>
        ...
        <div class="grid" style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
          <div class="form-group">
            ...
            <input type="datetime-local" id="start_time" name="start_time" value="{{ old('start_time', optional($assignment->start_time)->format('Y-m-d\TH:i')) }}" class="form-input" style="width:100%;padding:10px 12px;border:1px solid #D1D5DB;border-radius:8px;">
          ...
          <div class="form-group">
            ...
            <input type="datetime-local" id="end_time" name="end_time" value="{{ old('end_time', optional($assignment->end_time)->format('Y-m-d\TH:i')) }}" class="form-input" style="width:100%;padding:10px 12px;border:1px solid #D1D5DB;border-radius:8px;">
        ...
        <div class="form-actions" style="margin-top:20px;display:flex;gap:12px;">
          ...
          <button type="submit" class="btn btn-primary" style="background:#111827;color:#fff;border:none;padding:10px 16px;border-radius:8px;">Simpan Perubahan</button>
        </div>
      </form>
    </section>
  </div>
